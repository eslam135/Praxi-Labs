...
- Do **not** edit `ui/`, `core/SimulationLoop.ts`, or `rendering/SceneManager.ts` unless extending the framework deliberately.
...
- Prefer an `energy_drift` scalar via `energyDriftPercent` when total energy is meaningful.
...
Copy `PendulumExperiment.ts` for oscillatory systems, `ProjectileExperiment.ts` for trajectory-based systems, or `AtwoodExperiment.ts` for constrained 1-DOF mechanics.
